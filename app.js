...
app.get('/', (req, res) => {
  connection.query('SELECT * FROM crudimg_3', (error, results, fields) => {
...
app.get('/delete/:id', async (req, res) => {
  ...
  try {
    const [row] = await connection.promise().query('SELECT imagen FROM crudimg_3 WHERE id = ?', [id]);
    ...
    await connection.promise().query('DELETE FROM crudimg_3 WHERE id = ?', [id]);
    res.redirect('/');
  } catch (error) {
...
app.post('/save', upload.single('imagen'), async (req, res) => {
  ...
  try {
    ...
    await connection.promise().query(
      'INSERT INTO crudimg_3 SET ?',
      { nombre, apellidos, correo, fecha_nac, dni, imagen: s3Key }
    );
    res.redirect('/');
  } catch (error) {
...
app.get('/edit/:id', async (req, res) => {
  ...
  try {
    const [row] = await connection.promise().query('SELECT * FROM crudimg_3 WHERE id = ?', [id]);
    const crudimg_3 = row[0];

    if (crudimg_3 && crudimg_3.imagen) {
      const imageUrl = `/images/${crudimg_3.imagen}`; // URL de la imagen
      res.render('edit', { crudimg_3: crudimg_3, imageUrl: imageUrl });
    } else {
...
app.post('/update', upload.single('imagen'), async (req, res) => {
  ...
  try {
    await connection.promise().query(
      'UPDATE crudimg_3 SET nombre = ?, apellidos = ?, correo = ?, fecha_nac = ?, dni = ?, imagen = ? WHERE id = ?',
      [nombre, apellidos, correo, fecha_nac, dni, imagen, id]
    );
    res.redirect('/');
  } catch (error) {
...
app.get('/delete-all', async (req, res) => {
  try {
    const [rows] = await connection.promise().query('SELECT imagen FROM crudimg_3');
    ...
    await connection.promise().query('DELETE FROM crudimg_3');
